...
def main(cfg):
    ...
    training_args = transformers.TrainingArguments(
        ...
        eval_strategy="steps" if cfg.eval_while_train else "no",  # evaluation (origin
        seed=cfg.seed
    )
    ...
    if path_found:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            attn_implementation="flash_attention_2" if model_cfg.get("flash_attention2",
                                                                     "false") == "true" else "eager",
    ...
    if cfg.model_family == 'llama2-7b':
        ...
        if cfg.LoRA.targets == "all":
            ...
            raise NotImplementedError
    elif cfg.model_family == 'tinyllama':
        if cfg.LoRA.targets == "all":
            lora_targets = ['q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj']
        elif cfg.LoRA.targets == "self_attn":
            lora_targets = ['q_proj', 'k_proj', 'v_proj', 'o_proj']
    ...
    if cfg.importance_file:  # if using FILA
        ...
        importances = {n: torch.div(importance_f[n] / f_cnt, 1e-5 + (importance_r[n] / r_cnt)) for n in
                       importance_f.keys()}
